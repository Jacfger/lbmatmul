...
#pragma unroll
  for (size_t load_idx{0U};
    ...
    //        (int32_t)B_col_idx, (int32_t)(val & 0xF), (int32_t)(val >> 4),
    //        int(k), int(n), int(BLOCK_TILE_SIZE_K), int(BLOCK_TILE_SIZE_X));
  }
}
...
__global__ void gemm_v04(size_t m, size_t n, size_t k, T alpha, T const *A,
                         size_t lda, uint8_t const *B, size_t ldb, T beta, T *C,
                         size_t ldc) {
      ...
      for (size_t thread_tile_col_idx{0U};
        ...
        B_vals[thread_tile_col_idx] =
            B_thread_block_tile[B_thread_block_tile_row_idx]
                               [B_thread_block_tile_col_idx +
                                thread_tile_col_idx];
        uint8_t bv = B_thread_block_tile[B_thread_block_tile_row_idx]
                                        [B_thread_block_tile_col_idx +
                                         thread_tile_col_idx];
        // printf("shm[%d, %d], thread_col: %d, block_col_idx: %d, value: %d\n",
        //        (int32_t)B_thread_block_tile_row_idx,
        //        (int32_t)(B_thread_block_tile_col_idx + thread_tile_col_idx),
        //        (int32_t)thread_tile_col_idx,
        //        (int32_t)B_thread_block_tile_col_idx, (int32_t)(bv));
      }
...
#pragma unroll
          for (size_t thread_tile_col_idx{0U};
            ...
            C_thread_results[thread_tile_row_idx][thread_tile_col_idx] +=
                A_vals[thread_tile_row_idx] * static_cast<T>(b_val);
          }
        }
      }
    }
...
void launch_gemm_kernel_v04(size_t m, size_t n, size_t k, T const *alpha,
                            T const *A, size_t lda, uint8_t const *B,
