...
mat = torch.randint(
    -1000000000, 1000000000, (M // 1024 * 96, N), device=DEV, dtype=torch.int
)
...
torch.cuda.synchronize()
print("3bit:", (time.perf_counter() - tick) / COUNT)
...
torch.cuda.synchronize()
print("3bit:", (time.perf_counter() - tick) / COUNT, "(faster)")
...
mat = torch.randint(0, 1000000000 * 2, (M // 8, N), device=DEV, dtype=torch.int32)
tick = time.perf_counter()
for _ in range(COUNT):
    quant_ops.vec_mat4mul(vec, mat, mul, scales, zeros)
    torch.cuda.synchronize()
print("4bit:", (time.perf_counter() - tick) / COUNT, "(single token)")

mat = torch.randint(0, 16, (M // 2, N), device=DEV, dtype=torch.uint8)
COUNT = 1000
import time
...
tick = time.perf_counter()
for _ in range(COUNT):
    quant_ops.matmul_test(vec, mat, scales, zeros, mul)
torch.cuda.synchronize()
print("4bit:", (time.perf_counter() - tick) / COUNT)
